import { CLIENT_ROUTES, findClientRouteByPath } from "./client_routes.js";

function ensureNavRoot(){
  let root = document.getElementById("clientPortalNav");
  if(root) return root;

  root = document.createElement("nav");
  root.id = "clientPortalNav";
...
export function renderClientNav(){
  const root = ensureNavRoot();
  root.innerHTML = "";

  const title = document.createElement("div");
  title.textContent = "Client Portal Navigation";
  title.style.fontWeight = "700";
  title.style.marginBottom = "10px";
  root.appendChild(title);

  const current = findClientRouteByPath(location.pathname);

  CLIENT_ROUTES.forEach(item => {
